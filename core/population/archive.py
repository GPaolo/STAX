...
class Archive(object):
  ...
  def __setitem__(self, key, value):
    ...
    :return:
    """
    assert key in self.stored_info, 'Wrong key given. Available: {} - Given: {}'.format(self.stored_info, key)
    assert len(value) == self.size, 'List of values different from archive size. N of values: {} - Archive size: {}'.format(len(value), self.size)
    key_index = self.stored_info.index(key)
    for k in range(self.size):
      self.data[k][key_index] = value[k]
  ...
  def save(self, filepath, filename):
    ...
    """
    try:
      # Select everything except the view
      view_idx = self.stored_info.index('view')
      data = [[agent[i] if i != view_idx else None for i in range(len(self.stored_info))] for agent in self.data]

      with open(os.path.join(filepath, '{}_{}.pkl'.format(self.name, filename)), 'wb') as file:
        pkl.dump([data, self.filled_tracker], file)
