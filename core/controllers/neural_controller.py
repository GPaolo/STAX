...
class FFNeuralController(BaseController):
  ...
  def __init__(self, input_size, output_size, hidden_layers=2, hidden_layer_size=5, bias=True, **kwargs):
    ...
    if self.hidden_layers > 0:
      if self.layer_bias:
        self.genome_size = self.input_size*self.hidden_layer_size + self.use_bias*1 + \
                           (self.hidden_layer_size*self.hidden_layer_size + self.use_bias*1)*(self.hidden_layers-1) + \
                           self.hidden_layer_size*self.output_size + self.use_bias*1
      else:
        self.genome_size = self.input_size*self.hidden_layer_size + self.use_bias*self.hidden_layer_size + \
                           (self.hidden_layer_size*self.hidden_layer_size + self.use_bias*self.hidden_layer_size)*(self.hidden_layers-1) + \
                           self.hidden_layer_size*self.output_size + self.use_bias*self.output_size
    else:
      self.genome_size = self.input_size*self.output_size + self.use_bias*self.output_size
  # ----------------------------------
    ...
    if self.hidden_layers > 0:
      ...
      if self.use_bias:
        if self.layer_bias:
          ...
          self.bias.append(genome[idx:idx+self.hidden_layer_size])
          idx += self.hidden_layer_size
      # Hidden to hidden
      ...
      if self.use_bias:
        if self.layer_bias:
          ...
          self.bias.append(genome[idx:idx + self.output_size])
