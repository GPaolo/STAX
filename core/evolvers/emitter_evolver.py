...
class EmitterEvolver(BaseEvolver):
  ...
  def __init__(self, parameters, **kwargs):
    ...
    self.bounds = self.params.genome_limit * np.ones((self.genome_size, len(self.params.genome_limit))) # TODO remove cause not needed anymore
  ...
  def update_novelty_cand_buff(self, ns_ref_set, ns_pop, ns_off, emitter_idx, pool=None):
    """
    ...
    If the nov_cand_buffer flag is not set in the parameters the function returns without doing anything, no need to
    calculate the novelties in that case, nor to keep track of the most novel agent found so far.
    ...
    novelties = self.emitters[emitter_idx].pop['novelty']
