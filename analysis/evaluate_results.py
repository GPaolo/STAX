...
class EvalResults(object):
  """
  This class evaluates the results of a run by calculating the coverage, uniformity and rewards
  """
  def __init__(self, exp_path, agents=None):
    ...
    if self.params.env_name == 'Dummy':
      ...
      self.goals = 2
    elif self.params.env_name == 'CurlingDense':
      self.gt_bd_extractor = curling_gt_bd
      self.goals = 1
    elif self.params.env_name == 'Humanoid':
      self.gt_bd_extractor = humanoid_gt_bd
      self.goals = 0
    else:
  ...
  def rew_by_eval(self):
    ...
    rewards = {"rew_area_{}".format(i): [np.array([0, 0])] for i in range(self.goals)}
...
def main(path, args):
  arch_eval = EvalResults(path, agents=args.agents)
...
if __name__ == "__main__":
  ...
  #["-p", '/home/giuseppe/src/cmans/experiment_data/Curling_SERENE_no_add/2021_04_22_11:59_262639', '-g', '-1', '-abd'])
